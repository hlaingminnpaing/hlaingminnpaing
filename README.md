# 👋 Hi, I’m Hlaing Minn Paing

🔐 **Cybersecurity & Information Security Manager**  
🌏 Experience across Banking, Insurance & Telecom  
🛡️ Focused on Security Governance, Risk and Compliance, Managing the VAPT, DevSecOps, Infrastructure Security

---

## 🧠 About Me

I’m an Information Security professional with hands-on experience in:

- Cybersecurity Governance & Risk Management
- SOC, Incident Response & Threat Monitoring
- Vulnerability Assessment & Penetration Testing (VAPT)
- DevSecOps and Cloud Security
- Application Security using OWASP Top 10
- Security Awareness & Compliance (ISO 27001, NIST)

Currently working as **IT Security Manager**, supporting regional security operations and strategy.

---

## 🔧 Skills & Tools

**Security Domains**
- VAPT (Web, API, Infrastructure)
- Cloud Security (AWS / Azure)
- Kubernetes Security
- IAM & Zero Trust
- Incident Response <br>
- Security Architect

**Tools**
- Wazuh, Splunk SIEM
- Burp Suite, OWASP ZAP
- Nmap, Metasploit, Nessus, Qualys
- Trivy, SonarQube, Suricata
- Terraform, Kubernetes <br>
- Cloudflare, Trendmicro DeepSec, Trendmicro Apex One <br>

---

## 📂 Featured Repositories

🔹 **Cybersecurity Portfolio**  
👉 WIP

🔹 **Wazuh SIEM on Kubernetes**  
👉 WIP

🔹 **DevSecOps Tools on CI/CD Pipeline**  
👉 WIP

---

## 📜 Certifications

•	CISM ( ISACA ) <br>
•	ISO/IEC 27001 Lead Implementer ( Intertek ) <br>
•	CC ( ISC2 )<br>
•	EJPT ( eLearn Security )<br>
•	NSE 4 ( Fortinet )<br>
•	CCNA ( R&S, Security ) ( Cisco )<br>
•	EC Council Network Security Administrator ( EC Council )<br>
•	OCI Foundation Associate ( Oracle )<br>
•	Qualys Certified Specialist Vulnerability Management ( Qualys )<br>
•	Certified API Security Analyst ( API Security University )<br>
•	APISec Certified Practitioner ( API Security University )<br>
•	AZ 900,AZ 500 (In Progress)<br>


---

## ✍️ Writing & Knowledge Sharing

- Kubernetes Security Best Practices
- OWASP API Security Top 10
- SIEM & SOC Architecture

---

## 📫 Connect With Me

- 💼 LinkedIn: https://www.linkedin.com/in/hlaing-minn-paing/
- 📧 Email: hlaingminnpaing@gmail.com

> “Security is not a product, but a process.”
